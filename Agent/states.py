...
class File(BaseModel):
    ...
    purpose: str = Field(description="The purpose of the file, e.g. 'main application logic','data processing moduel', etc")

class Plan (BaseModel):
    name: str = Field(description="The name of app to be built")
    description: str = Field(
        description="A oneline description of the app to be built, e.g. 'A web application for managing person finance', etc.")
    techstack: str = Field(
...
class CoderState(BaseModel):
    ...
    skipped_files: Optional[list[str]] = Field(default_factory=list, description="Files that failed all retry attempts and were skipped, with their error messages")
    daily_quota_exhausted: bool = Field(False, description="True if any file was skipped due to Groq's daily (TPD) token quota being exhausted")